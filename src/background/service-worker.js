        ...
        switch (message.type) {
            ...
            case 'GET_USAGE': {
                ...
                if (usage.subscriptionEntityList && usage.subscriptionEntityList.length > 0) {
                    const monthlySubscriptions = usage.subscriptionEntityList.filter((sub) => sub.subscriptionPlan?.planType === 'MONTHLY' && sub.isActive);
                    // 优先级：PLUS > 其他非FREE > FREE（但我们不要FREE）
                    targetSubscription = monthlySubscriptions.find((sub) => sub.subscriptionPlan?.subscriptionName?.toUpperCase().includes('PLUS')) || monthlySubscriptions.find((sub) => !sub.subscriptionPlan?.subscriptionName?.toUpperCase().includes('FREE')) || null;
                }
